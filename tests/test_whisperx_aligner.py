...
from syncer.alignment.whisperx_aligner import WordAligner, AlignedWord, AlignmentResult
...
class TestWordAligner:
    ...
    def test_align_returns_words(self, mock_whisperx, tmp_path):
        ...
        assert result.words[0].score == 0.9
        assert result.words[1].word == "world"
        assert result.words[1].start == 1.1
        assert result.words[1].end == 1.5
        assert result.words[1].score == 0.0  # default for missing score
    ...
    def test_align_multi_segment(self, mock_whisperx, tmp_path):
        ...
        assert len(result.words) == 2
        assert result.words[0].word == "never"
        assert result.words[1].word == "gonna"
    ...
    def test_align_missing_start_end(self, mock_whisperx, tmp_path):
        ...
        assert len(result.words) == 1
        assert result.words[0].word == "partial"
        assert result.words[0].start == 0.0
        assert result.words[0].end == 0.0
        assert result.words[0].score == 0.0
    ...
    def test_align_calls_whisperx_correctly(self, mock_whisperx, tmp_path):
        ...
        mock_model.transcribe.return_value = {"language": "en", "segments": segments}
        ...
        mock_model.transcribe.assert_called_once_with(mock_audio, batch_size=8, language=None)
    ...
    def test_accepts_path_as_string(self, mock_whisperx, tmp_path):
        ...
        aligner = WordAligner()
        result = aligner.align(str(audio_path))  # pass as string

        assert result.words == []

    def test_align_passes_language_to_transcribe(self, mock_whisperx, tmp_path):
        """Explicit language is passed through to whisperx transcribe."""
        mock_model = self._setup_mocks(mock_whisperx)
        mock_model.transcribe.return_value = {"language": "hi", "segments": []}

        audio_path = _make_silent_wav(tmp_path / "hindi.wav")
        aligner = WordAligner()
        result = aligner.align(audio_path, language="hi")

        mock_model.transcribe.assert_called_once_with(
            mock_whisperx.load_audio.return_value, batch_size=8, language="hi"
        )
        assert result.detected_language == "hi"

    def test_align_returns_detected_language(self, mock_whisperx, tmp_path):
        """Detected language from transcribe result is returned."""
        mock_model = self._setup_mocks(mock_whisperx)
        mock_model.transcribe.return_value = {
            "language": "hi",
            "segments": [{"words": [{"word": "namaste", "start": 0.0, "end": 0.5, "score": 0.9}]}]
        }
        mock_whisperx.align.return_value = mock_model.transcribe.return_value

        audio_path = _make_silent_wav(tmp_path / "detect.wav")
        aligner = WordAligner()
        result = aligner.align(audio_path)
    ...
    def test_align_validates_invalid_language_code(self, mock_whisperx, tmp_path):
        """Invalid language code raises ValueError before calling transcribe."""
        self._setup_mocks(mock_whisperx)

        audio_path = _make_silent_wav(tmp_path / "invalid.wav")
        aligner = WordAligner()

        with pytest.raises(ValueError, match="Invalid language code"):
            aligner.align(audio_path, language="xyz123")

    def test_align_none_language_passes_none_to_transcribe(self, mock_whisperx, tmp_path):
        """language=None is passed as None to transcribe."""
        mock_model = self._setup_mocks(mock_whisperx)
        mock_model.transcribe.return_value = {"language": "en", "segments": []}

        audio_path = _make_silent_wav(tmp_path / "none_lang.wav")
        aligner = WordAligner()
        aligner.align(audio_path, language=None)

        mock_model.transcribe.assert_called_once_with(
            mock_whisperx.load_audio.return_value, batch_size=8, language=None
        )
...
def test_real_whisperx_silent_audio(tmp_path):
    ...
    assert len(result.words) <= 5  # generous threshold for silence detection
